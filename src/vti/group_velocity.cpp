 ...
 */
float SolverLove:: 
group_vel(const Mesh &M,float freq,
          float c,const float *egn) const
{
    return  get_love_group_vel(M.nglob_el,freq,c,egn,Mmat.data(),Kmat.data());
}
 ...
 */
scmplx SolverLove:: 
group_vel_att(const Mesh &M,float freq,
          scmplx c,const scmplx *egn) const
{
    return get_love_group_vel(M.nglob_el,freq,c,egn,Mmat.data(),CKmat.data());
}
                    ...
                    const T *Kmat)
{
    ...
    Eigen::Map<const Eigen::VectorX<T>> x(ul,ng),y(ur,ng);
          ...
          const float *ul) const
{
    ...
    return get_rayl_group_vel(ng,freq,c,ur,ul,Mmat.data(),Kmat.data());
}
...
{
    int ng = M.nglob_el*2 + M.nglob_ac;
    return get_rayl_group_vel(ng,freq,c,ur,ul,CMmat.data(),CKmat.data());
}
